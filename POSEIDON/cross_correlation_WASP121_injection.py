...
def cross_correlate(
    ...
    cs_p = interpolate.splrep(
        wl, F_p_conv, s=0.0
    )  # no need to times (R)^2 because F_p_obs, F_s_obs are already observed value on Earth
    ...
    for i in range(len(K_p_arr)):
        for j in range(len(V_sys_arr)):
            loglikelihood, CCF_sum = log_likelihood_sysrem(
                V_sys_arr[j],
                K_p_arr[i],
                dPhi,
                cs_p,
...
def batch_cross_correlate(core_index, args):
    ...
    for i in range(core_index, len(V_sys_arr_split), N_cores):
        ...
        uncertainties = pickle.load(
            open(data_path + "/uncertainties_injection.pic", "rb")
        )
        ...
        residuals, Us = fast_filter(data_norm, uncertainties, iter=5)
        ...
        log_L_arr, CCF_arr = cross_correlate(
            spectrum,
            continuum,
            wl,
            K_p_arr,
        ...
        pickle.dump(
            [V_sys_arr_split[i], K_p_arr, log_L_arr, CCF_arr],
            open(output_path + "/test_sysrem_" + str(i) + ".pic", "wb"),
        )  # N_cores of these produced. Will be read in by plot_CCF.py
...
if __name__ == "__main__":
    ...
    time_1 = time.time()
    pool.starmap(
        batch_cross_correlate, zip(core_indices, repeat((spectrum, continuum, wl)))
    )
    time_2 = time.time()
    print(time_2 - time_1)
