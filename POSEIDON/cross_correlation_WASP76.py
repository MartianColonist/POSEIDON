...
def cross_correlate(spectrum, continuum, wl, K_p_arr, V_sys_arr, wl_grid, residuals, Bs, V_bary, Phi, tmodel, uncertainties):
    dPhi = 0.0
    a = 1
    b = 0.7652
    # W_conv = 4.44
    ...
    for i in range(len(K_p_arr)):
        for j in range(len(V_sys_arr)):
            loglikelihood, CCF_sum = log_likelihood_sysrem(V_sys_arr[j], K_p_arr[i], dPhi, cs_p, wl_grid, residuals, Bs, V_bary, Phi, tmodel, uncertainties, a, b)
...
def batch_cross_correlate(core_index, args):
    ...
    for i in range(core_index, len(V_sys_arr_split), N_cores):
        ...
        log_L_arr, CCF_arr = cross_correlate(spectrum, continuum, wl, K_p_arr, V_sys_arr_split[i], wl_grid, residuals, Bs, V_bary, Phi, tmodel, uncertainties)
...
if __name__ == '__main__':
    ...
    data_dir = './reference_data/observations/WASP-76b'
