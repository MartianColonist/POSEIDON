...
def run_retrieval(
    ...
    # For high res
    high_res = data.get("high_res")  # True or None
    ...
    if sampling_algorithm == "MultiNest":
        ...
        if rank == 0:
            ...
            if disable_atmosphere == False:
                ...
                write_retrieved_PT(
                    retrieval_name, P, T_low2, T_low1, T_median, T_high1, T_high2
                )
...
def forward_model(
    ...
    (
        ...
        high_res_params,
    ) = split_params(param_vector, N_params_cum)
    ...
    if (disable_atmosphere == True) and ("transmission" in spectrum_type):
        ...
        if "log_P_ref" in physical_param_names:
            P_ref = np.power(
                10.0,
                physical_params[np.where(physical_param_names == "log_P_ref")[0][0]],
            )
        else:
            P_ref = P_ref_set
        ...
        if surface == True:
            P_surf = np.power(
                10.0,
                physical_params[np.where(physical_param_names == "log_P_surf")[0][0]],
            )
        else:
            P_surf = None
        ...
        if ("emission" in spectrum_type) and (spectrum_type != "direct_emission"):
            spectrum = compute_spectrum(
                ...
                opac,
                wl,
                spectrum_type=("direct_" + spectrum_type),
            )  # Always Fp (even for secondary eclipse)

        # For transmission spectra
        else:
            spectrum = compute_spectrum(
                planet, star, model, atmosphere, opac, wl, spectrum_type, y_p=y_p
            )
    ...
    if data.get("high_res"):  # For high res
        return 0, spectrum, atmosphere
    ...
    if "transmission" in spectrum_type:
        if stellar_contam != None:
            if "one_spot" in stellar_contam:
                ...
                I_het = I_het_grid[
                    closest_index(
                        T_het, T_het_grid[0], T_het_grid[-1], len(T_het_grid)
                    ),
                    closest_index(
                        log_g_het,
                        log_g_het_grid[0],
                ...
                I_phot = I_phot_grid[
                    closest_index(
                        T_phot, T_phot_grid[0], T_phot_grid[-1], len(T_phot_grid)
                    ),
                    closest_index(
                        log_g_phot,
                        log_g_phot_grid[0],
                ...
                I_spot = I_het_grid[
                    closest_index(
                        T_spot, T_het_grid[0], T_het_grid[-1], len(T_het_grid)
                    ),
                    closest_index(
                        log_g_spot,
                        log_g_het_grid[0],
                ...
                I_fac = I_het_grid[
                    closest_index(
                        T_fac, T_het_grid[0], T_het_grid[-1], len(T_het_grid)
                    ),
                    closest_index(
                        log_g_fac,
                        log_g_het_grid[0],
...
def CLR_Prior(chem_params_drawn, limit=-12.0):
    ...
    prior_lower_CLR = ((n - 1.0) / n) * (
        limit * np.log(10.0) + np.log(n - 1.0)
    )  # Lower limit corresponds to species underabundant
    prior_upper_CLR = ((1.0 - n) / n) * (
        limit * np.log(10.0)
    )  # Upper limit corresponds to species dominant
    ...
    for i in range(n):
        CLR[1 + i] = (
            chem_params_drawn[i] * (prior_upper_CLR - prior_lower_CLR)
        ) + prior_lower_CLR

    if np.abs(np.sum(CLR[1:n])) <= prior_upper_CLR:  # Impose same prior on X_0
        CLR[0] = -1.0 * np.sum(
            CLR[1:n]
        )  # CLR variables must sum to 0, so that X_i sum to 1

        if (np.max(CLR) - np.min(CLR)) <= (
            -1.0 * limit * np.log(10.0)
        ):  # Necessary for all X_i > 10^(-12)
            normalisation = np.sum(np.exp(CLR))

            for i in range(n + 1):
                # Map log-ratio parameters to mixing ratios
                X[i] = (
                    np.exp(CLR[i]) / normalisation
                )  # Vector of mixing ratios (should sum to 1!)

                # One final check that all X_i > 10^(-12)
                if X[i] < 1.0e-12:
                    return np.ones(n + 1) * (
                        -50.0
                    )  # Fails check -> return dummy array of log values
        ...
        np.abs(np.sum(CLR[1:n])) > prior_upper_CLR
    ):  # If falls outside of allowed triangular subspace
        return np.ones(n + 1) * (
            -50.0
        )  # Fails check -> return dummy array of log values


def PyMultiNest_retrieval(
    planet,
    star,
    model,
    opac,
    ...
    **kwargs
    ...
    def Prior(cube, ndim, nparams):
        ...
        for i, parameter in enumerate(param_names):
            ...
            # Draw mixing ratio parameters with uniform priors
                ...
                for species_q in param_species:
                    phrase = "_" + species_q
                    if (phrase + "_" in parameter) or (
                        parameter[-len(phrase) :] == phrase
                    ):
                        species = species_q
                ...
                # For 2D models, the prior range for 'Delta' parameters can change to satisfy mixing ratio priors
                    ...
                    # Mixing ratio gradient parameter comes second
                        ...
                        largest_delta = 2 * min(
                            (sampled_abundance - min_prior_abs),
                        ...
                        cube[i] = (
                            cube[i] * (max_value_delta - min_value_delta)
                        ) + min_value_delta
                ...
                # For 3D models, the prior ranges for 'Delta' parameters can change to satisfy mixing ratio priors
                    ...
                    if (species in species_EM_gradient) and (
                        ...
                        # Day-night mixing ratio gradient parameter comes third
                            ...
                            max_term_abundance = cube[i - 2] + abs(
                                cube[i - 1] / 2.0
                            )  # log_X_term_bar + |delta_log_X_term|/2
                            ...
                            # atmospheric regions satisfy the absolute abundance constraint
                            largest_delta = 2 * min(
                                (min_term_abundance - min_prior_abs),
                            ...
                            cube[i] = (
                                cube[i] * (max_value_delta_DN - min_value_delta_DN)
                            ) + min_value_delta_DN
        ...
        if "CLR" in prior_types.values():
            ...
            limit = prior_ranges[X_params[0]][
                0
            ]  # Same for all CLR variables, so choose first one
            ...
            if log_X[1] == -50.0:
                ...
                allowed_simplex = (
                    1  # Likelihood will be computed for this parameter combination
                )
    ...
    def LogLikelihood(cube, ndim, nparams):
        ...
        _, _, _, _, _, stellar_params, _, _, high_res_params = split_params(
            cube, N_params_cum
        )
        ...
        ymodel, spectrum, _ = forward_model(
            cube,
            planet,
            star,
            model,
...
def retrieved_samples(
    ...
    for i in range(N_sample_draws):
        ...
        ymodel, spectrum, atmosphere = forward_model(
            param_vector,
            planet,
            star,
            model,
        ...
        if i == 0:
            ...
            if disable_atmosphere == False:
                ...
                log_X_stored = np.zeros(
                    shape=(N_sample_draws, N_species, N_D, N_sectors, N_zones)
                )
    ...
    if disable_atmosphere == False:
        ...
        for q in range(N_species):
            (
                ...
                log_X_high2[q, :],
                _,
            ) = confidence_intervals(N_sample_draws, log_X_stored[:, q, :, 0, 0], N_D)
    ...
    (
        ...
        spec_high2,
        _,
    ) = confidence_intervals(N_sample_draws, spectrum_stored, len(wl))
...
def Bayesian_model_comparison(
    ...
    analyzer_1 = pymultinest.Analyzer(
        n_params_1, outputfiles_basename=model_1_prefix, verbose=False
    )
    analyzer_2 = pymultinest.Analyzer(
        n_params_2, outputfiles_basename=model_2_prefix, verbose=False
    )
    ...
    if Bayes_factor > 1:
        ...
        print(
            "No detection of the reference model, model "
            + model_2_name
            + " is preferred.\n"
        )
