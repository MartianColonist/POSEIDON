...
def interp_GPU(x_value, x, y):
    ...
    y_interp = y[prior_index] + (
        ((y[prior_index + 1] - y[prior_index]) / (x[prior_index + 1] - x[prior_index]))
...
def read_data(
    ...
    if spectrum_unit in [
        ...
        err = (
            2.0 * (np.array(data[3]) / np.array(data[2]))
        ) * spectrum  # Error propagation for Rp/Rs -> (Rp/Rs)^2
    else:
...
def read_high_res_data_deprecate(
    data_dir, method="sysrem", spectrum_type="transmission"
):
    """
    Read an external dataset file. The expected file format is:
...
def read_spectrum(fname, wl_unit="micron", skiprows=None):
    ...
    # Open file
    data = pd.read_csv(
        fname, sep="[\\s]{1,20}", header=None, skiprows=skiprows, engine="python"
    )
...
def read_PT_file(
    ...
    if PT_file_dir == "Tutorial/TRAPPIST-1e":
        PT_file_dir = os.path.abspath(
            os.path.join(
                os.path.dirname(__file__), ".", "reference_data/models/TRAPPIST-1e/"
            )
        )

    PT_file = pd.read_csv(
        PT_file_dir + "/" + PT_file_name,
        sep="[\\s]{1,20}",
        header=None,
        skiprows=skiprows,
        engine="python",
...
def read_chem_file(
    ...
    chem_file_name,
    P_grid,
    chem_species_in_file,
    chem_species_in_model,
    P_unit="bar",
    skiprows=None,
):
    """
    ...
    if chem_file_dir == "Tutorial/TRAPPIST-1e":
        chem_file_dir = os.path.abspath(
            os.path.join(
                os.path.dirname(__file__), ".", "reference_data/models/TRAPPIST-1e/"
            )
        )

    chem_file_input = pd.read_csv(
        chem_file_dir + "/" + chem_file_name,
        sep="[\\s]{1,20}",
        header=None,
        skiprows=skiprows,
        engine="python",
    ...
    for q in range(len(chem_species_in_model)):
        ...
        if species in chem_species_in_file:
            ...
            chem_interp = Interp(
                ...
                kind="linear",
                bounds_error=False,
                fill_value=(X_raw[-1, idx], X_raw[0, idx]),
            )
...
def bin_spectrum(wl_native, spectrum_native, R_bin, err_data=[]):
    ...
    if len(err_data) != 0:
        spectrum_binned, err_binned = spectres(
            wl_binned, wl_native, spectrum_native, spec_errs=err_data, verbose=False
        )
    ...
    # Call Spectres routine
        ...
        spectrum_binned = spectres(wl_binned, wl_native, spectrum_native, verbose=False)
...
def write_retrieved_spectrum(
    retrieval_name, wl, spec_low2, spec_low1, spec_median, spec_high1, spec_high2
):
    """
    ADD DOCSTRING
    """
    ...
    # Write top line
    f.write(
        "wl (μm) | spectrum: -2σ | spectrum: -1σ | spectrum: median "
...
def write_retrieved_PT(retrieval_name, P, T_low2, T_low1, T_median, T_high1, T_high2):
    """
    ADD DOCSTRING
    """
    ...
    for i in range(len(P)):
        f.write(
            "%.8e %.8e %.8e %.8e %.8e %.8e \n"
            % (P[i], T_low2[i], T_low1[i], T_median[i], T_high1[i], T_high2[i])
        )

    f.close()


def write_retrieved_log_X(
    retrieval_name,
    chemical_species,
    P,
    log_X_low2,
...
def read_retrieved_spectrum(planet_name, model_name, retrieval_name=None):
    ...
    spec_file = pd.read_csv(
        fname, sep="[\\s]{1,20}", engine="python", header=None, skiprows=1
    )
...
def confidence_intervals(sample_draws, array, length, integer=False):
    ...
    if length > 0:
        ...
        for i in range(length):
            ...
            arr_ordered_interp = lambda x: np.interp(
                x,
                arr_ordered[:, 0],
                arr_ordered[:, 1],
                left=arr_ordered[0, 1],
                right=arr_ordered[-1, 1],
            )
...
def write_samples_file(samples, param_names, n_params, results_prefix):
    ...
    np.savetxt(results_prefix + "_samples.txt", samples, comments="", header=header)
...
def generate_latex_param_names(param_names):
    ...
    for param in param_names:
        ...
        components = (
            []
        )  # Array of 'special' components for this parameter (e.g. 'log', 'bar')
        ...
        captured_characters = np.zeros(len(param)).astype(
            np.int64
        )  # Stays zero for entries with solo letters (e.g. 'H' in 'H2O')
        ...
        for letter in greek_letters_low:
            if (
                letter == "eta"
            ):  # Special check for 'eta', since  contained in 'beta' and 'theta'
                if (
                    (letter in param)
                    and ("theta" not in param)
                    and ("beta" not in param)
                ):
                    idx = find_str(
                        param, letter
                    )  # Find index where Greek letter starts
                    idxs += [idx]
        ...
        for phrase in phrases:
            if (
                phrase == "p"
            ):  # Special check for 'p' to avoid double counting phrases containing 'p'
                if "_p_" in param:
                ...
                phrase == "het"
            ):  # Special check for 'het' to avoid double counting phrases containing 'het'
                if "_het" in param:
                    idx = find_str(param, phrase)  # Find index where phrase starts
                    idxs += [idx]
                    components += ["phrase"]
        ...
        for i in range(len(components_sort)):
            if components_sort[i] == "log":
                latex_name += (
                    "\\" + param[idxs_sort[i] : idxs_sort[i] + lens_sort[i]] + " \\, "
                )
                ...
                if letter_digit_bracket_open == 1:
                    if N_letter_digit >= 2:
                        if components_sort[i] == "digit":
                            ...
                            latex_name += param[
                                idxs_sort[i] : idxs_sort[i] + lens_sort[i]
                            ]
...
def write_summary_file(
    ...
    max_param_len = len(
        max(param_names, key=len)
    )  # Used to align '=' signs for parameters
    ...
    lines += [
        ...
        "\n",
        "-> ln Z = "
        + stats_formatter.format(ln_Z)
        + " +/- "
        + stats_formatter.format(ln_Z_err)
        + "\n",
        "\n",
        "#################################\n",
    ]
    ...
    for i, param in enumerate(param_names):
        sig_m, centre, sig_p, formatter, unit = return_quantiles(
            stats, param, i, radius_unit, quantile="1 sigma"
        )

        lines += [
            param
            + " " * (max_param_len + 1 - len(param))
            + "=   "
            + formatter.format(centre)  # Handles number of spaces before equal sign
            + " (+"
            + formatter.format(sig_p)
            + ") (-"
            + formatter.format(sig_m)
            + ") "
            + unit
            + "\n"
        ]
    ...
    for i, param in enumerate(param_names):
        sig_m, centre, sig_p, formatter, unit = return_quantiles(
            stats, param, i, radius_unit, quantile="2 sigma"
        )
    ...
    for i, param in enumerate(param_names):
        sig_m, centre, sig_p, formatter, unit = return_quantiles(
            stats, param, i, radius_unit, quantile="3 sigma"
        )
    ...
    for i, param in enumerate(param_names):
        sig_m, centre, sig_p, formatter, unit = return_quantiles(
            stats, param, i, radius_unit, quantile="5 sigma"
        )
    ...
    for i, param in enumerate(param_names):
        _, _, _, formatter, unit = return_quantiles(
            stats, param, i, radius_unit
        )  # We only need the formatter and unit for the best fit parameters

        lines += [
            param
            + " " * (max_param_len + 1 - len(param))
            + "=   "
            + formatter.format(  # Handles number of spaces before equal sign
                best_fit_params[i]
            )
            + " "
            + unit
...
def write_MultiNest_results(
    planet, model, data, retrieval_name, N_live, ev_tol, sampling_algorithm, wl, R
):
    """
    ...
    for im, m in enumerate(mols):
        ...
        for i in range(len(refs)):
            ...
            if (vmr, hisig, losig) == (-1, -1, -1):
                ...
                if i != len(refs) - 1:
                    if m == "R_p_ref":
                        line += "${:.3f}^{{+{:.1f}}}_{{{:.1f}}}$ & ".format(
                            vmr, hisig, losig
                        )
                    elif m == "f_het":
                        line += "${:.2f}^{{+{:.2f}}}_{{{:.2f}}}$ & ".format(
                            vmr, hisig, losig
                        )
                    elif m in ["T_het", "T_spot", "T_fac", "T_phot", "T", "T_ref"]:
                        line += "${:.0f}^{{+{:.0f}}}_{{{:.0f}}}$ & ".format(
                            vmr, hisig, losig
                        )
                    elif "log" in m:
                        line += "${:.2f}^{{+{:.2f}}}_{{{:.2f}}}$ & ".format(
