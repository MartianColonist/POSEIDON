...
def plot_parameter_panel(ax, param_vals, N_bins, param, param_min, param_max, colour):
    # Plot histogram
    _, _, low1, median, high1, _, _ = plot_histogram(
        N_bins, param_vals, colour, ax, 0.0
    )
    ...
    for q in range(len(plot_parameters)):
        ...
        for m in range(N_models):
            ...
            low1, median, high1 = plot_parameter_panel(
                ax, param_vals_m[:, q], N_bins[q], param, param_min, param_max, colour
            )
        ...
        if (retrieval_codes == []) or (retrieval_codes[m] == "POSEIDON"):
            ...
            for i in range(N_samples):
                if "H2" and "He" in bulk_species:
                    ...
                    _, _, log_X_stored[i, 2:], _, _, _, _, _ = split_params(
                        samples[i], N_params_cum
                    )

                    # Add H2 and He mixing ratios
                    X_H2 = (1.0 - np.sum(np.power(10.0, log_X_stored[i, 2:]))) / (
                        1.0 + He_fraction
                    )
                    ...
                    # Extract mixing ratios from MultiNest samples
                    _, _, log_X_stored[i, 1:], _, _, _, _, _ = split_params(
                        samples[i], N_params_cum
                    )
...
def plot_retrieved_element_ratios(
    X_vals,
    all_species,
    plot_ratios,
    colour_list,
    ...
    for q in range(len(plot_ratios)):
        ...
        for m in range(len(X_vals)):
            ...
            ratio_vals = elemental_ratio(
                all_species, X_vals_m, elements[0], elements[1]
            )
            ...
            if elements[1] == "H":
                low1, median, high1 = plot_parameter_panel(
                    ax, np.log10(ratio_vals), ratio, N_bins, x_min, x_max, colour
                )
            else:
                low1, median, high1 = plot_parameter_panel(
                    ax, ratio_vals, ratio, N_bins, x_min, x_max, colour
                )
        ...
        if elements[1] == "H":
            ax.set_xlabel(
                r"$\log ($" + r"$\mathrm{" + ratio + r"}$" + r") ($\times \rm{solar})$",
                fontsize=12,
            )
        else:
