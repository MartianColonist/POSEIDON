...
warnings.filterwarnings(
    "ignore",
    message="Attempted to set non-positive left "
...
warnings.filterwarnings(
    "ignore",
    message="This figure includes Axes that are "
...
def plot_transit(
    ...
    if N_sectors_init > 1:
        r_pole = 0.5 * (
            r[:, (N_sectors_init // 2) - 1, :] + r[:, (N_sectors_init // 2), :]
        )  # Average sectors adjacent to north pole
        T_pole = 0.5 * (
            T[:, (N_sectors_init // 2) - 1, :] + T[:, (N_sectors_init // 2), :]
        )  # Average sectors adjacent to north pole
    else:
        r_pole = r[:, 0, :]
        T_pole = T[:, 0, :]
    ...
    if N_zones_init > 1:
        r_term = 0.5 * (
            r[:, :, (N_zones_init // 2) - 1] + r[:, :, (N_zones_init // 2)]
        )  # Average zones adjacent to terminator plane
        T_term = 0.5 * (
            T[:, :, (N_zones_init // 2) - 1] + T[:, :, (N_zones_init // 2)]
        )  # Average zones adjacent to terminator plane
    else:
        r_term = r[:, :, 0]
        T_term = T[:, :, 0]
    ...
    if perspective == "terminator":
        ...
        if len(phi) == 1:
            ...
            phi_prime_S = (-1.0 * phi_prime_N)[
                ::-1
            ]  # South pole symmetric about y axis
        ...
        for j_all in range(N_sectors):  # Twice to cover North and South pole sectors
            ...
            planet_core = Circle(
                (0.0, 0.0), r[0, 0, 0] / R_p, facecolor="#1E202C", edgecolor="None"
            )
        ...
        if annotate_Rp == True:
            ax.annotate(
                s="",
                xy=(0.0, 0.0),
                xytext=(-1.0 / np.sqrt(2), -1.0 / np.sqrt(2)),
                arrowprops=dict(arrowstyle="<->", color="white", alpha=1.0),
    ...
    # Slice through the north-south pole plane
        ...
        if len(theta) == 1:
            ...
            theta_prime_edge_all = theta_prime_edge_all[
                :-1
            ]  # Remove boundaries at equator
        ...
        for k_all in range(N_zones):  # Twice to cover North and South pole sectors
            ...
            for i in range(0, N_layers - 11, 11):
                planet_atm = Wedge(
                    (0.0, 0.0),
                    r_pole[i + 11, k] / R_p,
                    theta_prime_edge_all[
                        k_all + 1
                    ],  # Wedges plots w.r.t x axis and in degrees
                    theta_prime_edge_all[k_all],
                    edgecolor="None",
            ...
            # Plot planet core (circular, below atmosphere)
            planet_core = Circle(
                (0.0, 0.0), r[0, 0, 0] / R_p, facecolor="black", edgecolor="None"
            )
            ax.add_artist(planet_core)
...
def plot_geometry_spectrum_mixed(
    planet,
    star,
    model,
    atmosphere,
        ...
        # Evening-Morning gradient
            ...
            for profile_to_plot in show_profiles:
                ...
                if profile_to_plot == "morning":
                    ax.semilogy(
                        T[:, -1, 0], P, lw=1.5, color="darkblue", label="Morning"
                    )
    ...
    # 3D temperature profile
        ...
        for profile_to_plot in show_profiles:
            ...
            if profile_to_plot == "evening-day":
                ax.semilogy(
                    T[:, 0, 0], P, lw=1.5, color="crimson", label="Evening: Day"
                )
            ...
            if profile_to_plot == "morning-terminator":
                ax.semilogy(
                    ...
                    ls=":",
                    color="darkblue",
                    label="Morning: Terminator",
                )
            if profile_to_plot == "terminator-average":
                ax.semilogy(
                    0.25 * (T[:, 0, 0] + T[:, 0, -1] + T[:, -1, 0] + T[:, -1, -1]),
                    P,
                    lw=1.5,
                    ls="--",
                    color="darkorange",
                    label="Terminator Average",
                )
...
def plot_chem(
    ...
    if len(plot_species) > 8:
        raise Exception(
            "Max number of concurrent species on plot is 8.\n"
    ...
    if log_X_min == None:
        ...
        for q, species in enumerate(plot_species):
            log_X_min = min(
                log_X_min,
                (
                    np.floor(np.min(log_X[chemical_species == species, :, 0, 0][0]))
                    - 1.0
                ),
            )
            ...
            log_X_max = max(
                ...
                (
                    min(
                        (
                            np.ceil(
                                np.max(log_X[chemical_species == species, :, 0, 0][0])
                            )
                            + 1.0
                        ),
                        0.0,
    ...
    if Atmosphere_dimension == 1:
        for q, species in enumerate(plot_species):
            ax.semilogy(
                log_X[chemical_species == species, :, 0, 0][0],
                P,
                lw=1.5,
                color=colours[q],
    ...
    # 2D temperature profile
        ...
        if len(show_profiles) == 0:
            raise Exception(
                "For 2D or 3D models, you need to specify " + "which regions to plot."
            )
                ...
                for profile_to_plot in show_profiles:
                    ...
                    if profile_to_plot == "morning-day":
                        ax.semilogy(
                            ...
                            P,
                            lw=1.5,
                            ls=linestyles["loosely dashdotted"],
                            color=colours[q],
                    ...
                    if profile_to_plot == "morning-terminator":
                        ax.semilogy(
                            ...
                            * (
                            ...
                            P,
                            lw=1.5,
                            ls=linestyles["densely dashdotted"],
                            color=colours[q],
...
def set_spectrum_wl_ticks(wl_min, wl_max, wl_axis="log"):
    ...
    if wl_range > 0.2:
        if wl_max <= 1.0:
            wl_ticks_1 = np.arange(
                round_sig_figs(wl_min, 1), round_sig_figs(wl_max, 2) + 0.01, 0.1
            )
            wl_ticks_2 = np.array([])
            wl_ticks_3 = np.array([])
            wl_ticks_4 = np.array([])
            ...
            if wl_min < 1.0:
                ...
                wl_ticks_2 = np.concatenate(
                    (
                        [round_sig_figs(wl_min, 2)],
                        np.arange(
                            np.ceil(wl_min * 5) / 5.0,
                            round_sig_figs(wl_max, 2) + 0.01,
                            0.2,
                        ),
                    )
                )
            ...
            if wl_min < 1.0:
                ...
                wl_ticks_2 = np.concatenate(
                    (
                        [round_sig_figs(wl_min, 2)],
                        np.arange(np.ceil(wl_min * 5) / 5.0, 3.0, 0.2),
                    )
                )
                if wl_axis == "log":
            ...
            if wl_min < 1.0:
                ...
                wl_ticks_2 = np.concatenate(
                    (
                        [round_sig_figs(wl_min, 2)],
                        np.arange(np.ceil(wl_min * 2) / 2.0, 3.0, 0.5),
                    )
                )
                if wl_axis == "log":
                    wl_ticks_3 = np.arange(3.0, round_sig_figs(wl_max, 2) + 0.01, 1.0)
            ...
            if wl_min < 1.0:
                ...
                if wl_axis == "log":
                    ...
                    wl_ticks_2 = np.concatenate(
                        (
                            [round_sig_figs(wl_min, 2)],
                            np.arange(np.ceil(wl_min), 3.0, 1.0),
                        )
                    )
                    wl_ticks_3 = np.arange(3.0, 10.0, 1.0)
                    wl_ticks_4 = np.arange(10.0, round_sig_figs(wl_max, 3) + 0.01, 1.0)
                ...
                if wl_axis == "log":
                    wl_ticks_3 = np.concatenate(
                        (
                            [round_sig_figs(wl_min, 2)],
                            np.arange(np.ceil(wl_min), 10.0, 1.0),
                        )
                    )
                ...
                wl_ticks_3 = np.array([])
                if wl_axis == "log":
                    wl_ticks_4 = np.concatenate(
                        (
                            [round_sig_figs(wl_min, 3)],
                            np.arange(
                                np.ceil(wl_min), round_sig_figs(wl_max, 3) + 0.01, 1.0
                            ),
                        )
                    )
                elif wl_axis == "linear":
    ...
    # For high-resolution (zoomed in) spectra
        ...
        wl_ticks = np.arange(
            round_sig_figs(wl_min, 3), round_sig_figs(wl_max, 3) + 0.0001, wl_spacing
        )
...
def plot_spectra(
    ...
    if y_unit in ["(Rp/Rs)^2", "(Rp/R*)^2", "(Rp/R*)", "transit_depth"]:
        ...
        raise Exception(
            "Unexpected y unit. Did you mean 'transit_depth' " + "or 'eclipse_depth'?"
        )
    ...
    if (N_spectra > 8) and len(colour_list) == 0:
        raise Exception(
            "Max number of concurrent spectra to plot is 8 with default colour list"
        )
    ...
    if (len(text_annotations) != 0) and (len(text_annotations) != len(annotation_pos)):
        raise Exception(
            "Number of annotation labels does not match provided positions."
        )
    ...
    if (data_properties != None) and (show_data == True):
        ...
        if ((N_datasets > 5) and len(data_colour_list) == 0) or (
            (N_datasets > 5) and len(data_marker_list) == 0
        ):
            raise Exception(
                "Max number of concurrent datasets to plot is 5 with default data colours and markers."
            )
        ...
        if (len(data_labels) != 0) and (N_datasets != len(data_labels)):
            raise Exception(
                "Number of dataset labels does not match number of datasets."
            )
        if (len(data_marker_list) != 0) and (N_datasets != len(data_marker_list)):
            raise Exception(
                "Number of dataset markers does not match number of datasets."
            )
        if (len(data_marker_size_list) != 0) and (
            N_datasets != len(data_marker_size_list)
        ):
            raise Exception(
                "Number of dataset marker sizes does not match number of datasets."
            )
        ...
        if (
            len(data_marker_size_list) == 0
        ):  # If user did not specify a custom colour list
            data_markers_size = [
                3,
                3,
                3,
    ...
    if figure_shape == "default":
        ...
        fig.set_size_inches(
            10.667, 6.0
        )  # 16:9 widescreen format (for two column figures)
    ...
    for i in range(N_spectra):
        ...
        if plot_full_res == True:
            ax1.plot(
                wl, spec, lw=0.5, alpha=0.4, zorder=i, color=colours[i], label=label_i
            )
    ...
    if ylabels == True:
        if plot_type == "transmission":
            if y_min_plt < 0.10:
                ax1.set_ylabel(
                    r"Transit Depth $(R_p/R_*)^2$", fontsize=y_label_fontsize
                )
            else:
            ...
            ax1.set_ylabel(
                r"$F_{\rm{p}}$ (W m$^{-2}$ m$^{-1}$)", fontsize=y_label_fontsize
            )
        elif (plot_type == "direct_emission") and (y_unit in ["Fs", "F*"]):
            ax1.set_ylabel(
                r"$F_{\rm{s}}$ (W m$^{-2}$ m$^{-1}$)", fontsize=y_label_fontsize
            )
...
def plot_data(
    ...
    if N_datasets == 0:
        raise Exception("Must provide at least one dataset to plot!")
    if ((N_datasets > 7) and len(data_colour_list) == 0) or (
        (N_datasets > 7) and len(data_marker_list) == 0
    ):
        raise Exception(
            "Max number of concurrent datasets to plot is 7 with default data colours and markers."
        )
    if (len(data_colour_list) != 0) and (N_datasets != len(data_colour_list)):
        raise Exception("Number of colours does not match number of datasets.")
    if (len(data_labels) != 0) and (N_datasets != len(data_labels)):
    ...
    if wl_min == None:
        wl_min = np.min(
            wl_data - 4 * bin_size
        )  # Minimum at twice the bin width for the shortest wavelength data
    else:
        wl_min = wl_min
    ...
    if wl_max == None:
        wl_max = np.max(
            wl_data + 4 * bin_size
        )  # Maximum at twice the bin width for the longest wavelength data
    else:
        wl_max = wl_max
    ...
    if (legend_box == True) and (legend_location != "outside right"):
        legend = ax1.legend(
            loc=legend_location, shadow=True, prop={"size": 10}, ncol=1, frameon=True
        )  # Legend settings
        frame = legend.get_frame()
        frame.set_facecolor("0.90")
    elif legend_location == "outside right":
        ...
        legend = ax1.legend(
            loc=legend_location, shadow=True, prop={"size": 10}, ncol=1, frameon=False
        )  # Legend settings
    ...
    if save_fig == True:
        if plt_label == None:
            ...
            file_name = output_dir + planet_name + "_" + plt_label + "_data.pdf"
...
def plot_spectra_retrieved(
    ...
    data_marker_size_list=[],
    data_alpha_list=[],
    data_eline_alpha_list=[],
    data_edge_width_list=[],
    data_eline_width_list=[],
    line_width_list=[],
    binned_colour_list=[],
    text_annotations=[],
    annotation_pos=[],
    ...
    if (N_spectra > 5) and (len(colour_list) == 0):
        raise Exception(
            "Max number of concurrent retrieved spectra to plot is 5 with default colour list."
        )
    if (len(colour_list) != 0) and (N_spectra != len(colour_list)):
        raise Exception("Number of colours does not match number of spectra.")
    if (len(binned_colour_list) != 0) and (N_spectra != len(binned_colour_list)):
        raise Exception(
            "Number of binned model colours does not match number of spectra."
        )
    if (len(spectra_labels) != 0) and (N_spectra != len(spectra_labels)):
        raise Exception("Number of model labels does not match number of spectra.")
    ...
    if N_datasets == 0:
        raise Exception("Must provide at least one dataset to plot!")
    if ((N_datasets > 10) and len(data_colour_list) == 0) or (
        (N_datasets > 10) and len(data_marker_list) == 0
    ):
        raise Exception(
            "Max number of concurrent datasets to plot is 10 with default data colours and markers."
        )
    if (len(data_colour_list) != 0) and (N_datasets != len(data_colour_list)):
        raise Exception("Number of colours does not match number of datasets.")
    if (len(data_labels) != 0) and (N_datasets != len(data_labels)):
    ...
    if (len(data_alpha_list) != 0) and (N_datasets != len(data_alpha_list)):
        raise Exception(
            "Number of dataset alpha values does not match number of datasets."
        )
    ...
    if (len(data_eline_width_list) != 0) and (N_datasets != len(data_eline_width_list)):
        raise Exception(
            "Number of error bar line widths does not match number of spectra."
        )
    if (len(text_annotations) != 0) and (len(text_annotations) != len(annotation_pos)):
        raise Exception(
            "Number of annotation labels does not match provided positions."
        ...
        if offset_datasets == "single_dataset":
            ...
            if data_properties["offset_1_end"] == 0:
                offset_start, offset_end = (
                    data_properties["offset_start"],
                    data_properties["offset_end"],
                )
            else:
                offset_start, offset_end = (
                    data_properties["offset_1_start"],
                    data_properties["offset_1_end"],
                )

            # Catch offsets for one dataset
            if isinstance(offset_start, np.int64):
                offset_start, offset_end = np.array([offset_start]), np.array(
                    [offset_end]
                )
            ...
            if verbose_offsets == True:
                if data_properties["offset_1_end"] == 0:
                    print(
                        "Applied " + str(delta_rel) + " ppm offset to offset_datasets"
                    )
                else:
                    print(
                        "Applied " + str(delta_rel) + " ppm offset to offset_1_datasets"
                    )

        # Add multiple offsets
        elif (offset_datasets == "two_datasets") or (
            offset_datasets == "three_datasets"
        ):
            ...
            for start_name, end_name in zip(offset_start_list, offset_end_list):
                offset_start, offset_end = (
                    data_properties[start_name],
                    data_properties[end_name],
                )
            ...
            for delta_rel, (offset_start, offset_end) in zip(
                delta_rel_array, offset_start_end
            ):
                # Note: offsets are in ppm
                ydata_to_plot[offset_start:offset_end] = (
                    ydata[offset_start:offset_end] - delta_rel * 1e-6
                )

            if verbose_offsets == True:
                print(
                    "Applied "
                    + str(delta_rel_array[0])
                    + " ppm offset to offset_1_datasets"
                )
                print(
                    "Applied "
                    + str(delta_rel_array[1])
                    + " ppm offset to offset_2_datasets"
                )

                if offset_datasets == "three_datasets":
                    print(
                        "Applied "
                        + str(delta_rel_array[2])
                        + " ppm offset to offset_3_datasets"
                    )
    ...
    if ylabels == True:
        if plot_type == "transmission":
            ...
            if y_unit == "eclipse_depth_ppm":
                ...
                ax1.set_ylabel(
                    r"Emission Spectrum $(F_p/F_*)$", fontsize=y_label_fontsize
                )
        elif plot_type == "direct_emission":
            ax1.set_ylabel(
                r"$F_{\rm{p}}$ (W m$^{-2}$ m$^{-1}$)", fontsize=y_label_fontsize
    ...
    if save_fig == True:
        if plt_label == None:
            ...
            file_name = (
                output_dir + planet_name + "_" + plt_label + "_retrieved_spectra.pdf"
            )
...
def plot_PT_retrieved(
    ...
    if Atmosphere_dimension > 1:
        raise Exception(
            "This function does not currently support " + "multidimensional retrievals."
        )
...
def plot_chem_retrieved(
    ...
    if (len(model_labels) != 0) and (N_chem != len(model_labels)):
        raise Exception(
            "Number of model labels does not match number of mixing ratio profiles."
        )
    for q, species in enumerate(plot_species):
        if species not in chemical_species:
            raise Exception(species + " not included in this model.")
    ...
    if Atmosphere_dimension > 1:
        ...
        for i in range(N_chem):
            ...
            for q, species in enumerate(plot_species):
                ...
                ax.semilogy(
                    log_X_med[chemical_species == species, :][0],
                    P,
                    lw=1.5,
                    color=colours[q],
                    label=label_med,
                ...
                ax.fill_betweenx(
                    P,
                    log_X_low1[chemical_species == species, :][0],
                ...
                if plot_two_sigma == True:
                    ax.fill_betweenx(
                        P,
                        log_X_low2[chemical_species == species, :][0],
                ...
                if log_X_true != None:
                    ax.semilogy(
                        log_X_true[chemical_species == species, :][0],
                        P,
                        lw=1.5,
                        color=colours[q],
                        ls=linestyles["dashed"],
...
def plot_histogram(nbins, vals, colour, ax, shrink_factor, x_max_array, alpha_hist):
    ...
    low3, low2, low1, median, high1, high2, high3 = confidence_intervals(
        len(vals), vals, 0
    )
...
def plot_parameter_panel(
    ax, param_vals, N_bins, param, param_min, param_max, colour, x_max_array, alpha_hist
):

    # Plot histogram
    _, low2, low1, median, high1, high2, _ = plot_histogram(
        N_bins, param_vals, colour, ax, 0.0, x_max_array, alpha_hist
    )
...
def plot_retrieved_parameters(
    axes_in,
    param_vals,
    plot_parameters,
    parameter_colour_list,
    retrieval_colour_list,
    retrieval_labels,
    span,
    truths,
    N_rows,
    ...
    for q in range(len(plot_parameters)):
        ...
        for m in range(N_models):
            ...
            if N_models == 1:
                ...
                colour = retrieval_colour_list[
                    m
                ]  # Each retrieval has a different colour
        ...
        for m in range(N_models):
            ...
            # Plot histogram
            low1, median, high1 = plot_parameter_panel(
                ax,
                param_vals_m[:, q],
                N_bins[q],
                param,
            ...
            if show_title == True:
                if m == 0:
                    ...
                    title = r"${{{0}}}_{{-{1}}}^{{+{2}}}$"
                    title = title.format(
                        fmt(median), fmt((median - low1)), fmt((high1 - median))
                    )
            ...
            if "T" in param:
                if (param_max - param_min) < 400:
                    xmajor_interval = 100
                    xminor_interval = 50
                elif ((param_max - param_min) >= 400) and (
                    (param_max - param_min) < 800
                ):
                    xmajor_interval = 200
                    xminor_interval = 100
                elif ((param_max - param_min) >= 800) and (
                    (param_max - param_min) < 2000
                ):
                    xmajor_interval = 400
                    xminor_interval = 200
            ...
            if "log_" in param:
                if (param_max - param_min) <= 1.0:
                    xmajor_interval = 0.4
                    xminor_interval = 0.1
                elif ((param_max - param_min) > 1.0) and (
                    (param_max - param_min) <= 2.0
                ):
                    xmajor_interval = 0.5
                    xminor_interval = 0.1
                elif ((param_max - param_min) > 2.0) and (
                    (param_max - param_min) <= 4.0
                ):
                    xmajor_interval = 1.0
                    xminor_interval = 0.2
                elif ((param_max - param_min) > 4.0) and (
                    (param_max - param_min) <= 8.0
                ):
                    xmajor_interval = 2.0
                    xminor_interval = 0.5
        ...
        if len(truths) != 0:
            ax.axvline(
                x=truths[q], linewidth=1.5, linestyle="-", color="crimson", alpha=0.8
            )

        if len(vertical_lines) != 0:
            for n in range(len(vertical_lines)):
                ax.axvline(
                    x=vertical_lines[n][q],
                    linewidth=1.5,
                    linestyle="-",
                    color=vertical_lines_colors[n],
                    alpha=0.8,
                )
...
def plot_histograms(
    ...
    if (N_models == 1) and (len(parameter_colour_list) == 0):
        ...
        if len(plot_parameters) != 0:
            if len(parameter_colour_list) != len(plot_parameters):
                raise Exception(
                    "Number of parameter colours does not match the "
        ...
        if len(retrieval_colour_list) != N_models:
            raise Exception(
                "Number of retrieval colours does not match the "
                + "number of retrieval models."
            )

    param_vals = (
        []
    )  # List to store parameter values for all models, samples, and parameters
    ...
    for m in range(N_models):
        ...
        if (len(retrieval_codes) == 0) or (retrieval_codes[m] == "POSEIDON"):
            ...
            if Atmosphere_dimension > 1:
                print(
                    "Note: this function is not currently configured for bulk gas "
            ...
            if disable_atmosphere == False:
                ...
                for i in range(N_samples):
                    ...
                    atmosphere_i = get_retrieved_atmosphere(
                        planet,
                        model,
                        np.logspace(np.log10(100.0), np.log10(1e-6), 100),
        ...
        for q in range(N_params_to_plot):
            ...
            if param == "log_P_top_slab_SiO2":
                try:
                    param_samples_m[:, q] = samples[
                        :, np.where(param_names == param)[0][0]
                    ]
                except:
                ...
                # Derived mean molecular weight samples
                    ...
                    if disable_atmosphere == True:
                        raise Exception(
                            "Error: can't plot the mean molecular weight "
                ...
                # Elemental ratios
                    ...
                    if disable_atmosphere == True:
                        raise Exception(
                            "Error: can't plot an elemental ratio for "
                        ...
                        if ratio == "M/H":
                            ...
                            for element_i in ["C", "O", "N", "P", "S"]:
                                element_ratio = elemental_ratio_samples(
                                    chemical_species, X_stored, element_i, element_2
                                )
                                element_ratio_norm += element_ratio / 10 ** (
                                    solar_abundances[element_i] - 12.0
                                )
                        else:
                            element_ratio = elemental_ratio_samples(
                                chemical_species, X_stored, element_1, element_2
                            )
                            if element_2 == "H":
                                element_ratio_norm = element_ratio / 10 ** (
                                    solar_abundances[element_1] - 12.0
                                )
                            else:
