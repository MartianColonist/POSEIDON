...
def cross_correlate(
    coord, K_p_arr, V_sys_arr, wl, planet_spectrum, star_spectrum, data
):
    print(coord)
    K_p = K_p_arr[coord[0]]
    V_sys = V_sys_arr[coord[1]]
    loglikelihood, CCF = loglikelihood_PCA(
        V_sys, K_p, 0, 1, wl, planet_spectrum, star_spectrum, data
    )
...
if __name__ == "__main__":
    ...
    results = Parallel(n_jobs=N_jobs, max_nbytes=1e7, verbose=10)(
        delayed(cross_correlate)(x, K_p_arr, V_sys_arr, wl, F_p_conv, F_s_conv, data)
